...
# Assuming StreamlitCropDiseaseAnalyzer is defined elsewhere in your code
# from your_module import StreamlitCropDiseaseAnalyzer
...
def main():
    ...
    if selected_crop:
        ...
        st.markdown(
            f'<div class="metric-card">Current Growth Stage: {growth_stage}</div>',
            unsafe_allow_html=True
        )

        npk_requirements = analyzer.get_npk_requirements(selected_crop, growth_stage)
        st.markdown('<h3 class="section-header">NPK Requirements</h3>', unsafe_allow_html=True)

        # Organize NPK display
        npk_cols = st.columns(3)
        for nutrient, value in npk_requirements.items():
            with npk_cols[list(npk_requirements.keys()).index(nutrient)]:
                st.markdown(f'<div class="metric-card">{nutrient}: {value} kg/acre</div>', unsafe_allow_html=True)

        # Additional recommendations
        recommendations = analyzer.get_recommendations(selected_crop)
