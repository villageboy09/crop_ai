...
def main():
    ...
    if selected_crop:
        ...
        npk_requirements = analyzer.calculate_npk_requirements(selected_crop, acres, growth_stage)

        # Display NPK requirements in a user-friendly format
        st.markdown('<h2 class="section-header">NPK Requirements</h2>', unsafe_allow_html=True)
        for nutrient, amount in npk_requirements.items():
            st.markdown(f'<div class="recommendation-card">🌱 {nutrient}: {amount} kg</div>', unsafe_allow_html=True)
